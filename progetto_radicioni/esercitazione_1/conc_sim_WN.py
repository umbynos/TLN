...
def main():
	...
	global depth_max
	depth_max = depthMax()
	print("profondita max", depth_max)
	print("shortest_path_sim",shortest_path_sim("tiger","tiger"))
	...
	for i in range(len(words_array)):
		...
		shortest_path_sim_indexes.append(shortest_path_sim(word1,word2))
		print("shortest_path_sim:", shortest_path_sim_indexes[i])
...
# shortest_path_sim(word1, word2) returns a similarity based on the shortest path length from word1 to word2 considering the two senses that give the minimum path
# If no senses are found, the similarity is considered to be zero
def shortest_path_sim(word1, word2):
	synset1 = wn.synsets(word1)
	synset2 = wn.synsets(word2)
	sim = None
	sht_path = None
...
# shortest_path(sense1, sense2) returns the minum paths between two senses
